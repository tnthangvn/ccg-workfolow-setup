---
description: 'Multi-model Test Generation: Intelligent routing to claude for backend tests / antigravity for frontend tests'
---
...
- Intelligent routing: Backend → claude, Frontend → antigravity, Full-stack → parallel
- Comply with existing project test frameworks and styles

## Your role

You are the **test engineer**, orchestrating the test generation flow:
- **claude** – Backend test generation (**backend authoritative**)
...
| Model | Prompt |
|------|--------|
| Backend | `/home/thangtn/.claude/.ccg/prompts/claude/tester.md` |
...
- **Backend code** → `Bash({ command: "...--backend claude...", run_in_background: false })`
  - ROLE_FILE: `/home/thangtn/.claude/.ccg/prompts/claude/tester.md`
...
- **Full-stack code** → Call both in parallel:
  1. `Bash({ command: "...--backend claude...", run_in_background: true })`
     - ROLE_FILE: `/home/thangtn/.claude/.ccg/prompts/claude/tester.md`
...
2. **Intelligent Routing** – Backend tests via claude, frontend tests via antigravity.
