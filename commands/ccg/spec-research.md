...
- Use `mcp__fast-context__fast_context_search` (or fallback search tools if fast-context is unavailable) to minimize manual grep/find operations.
...
2. **Initial Codebase Assessment**
   - Use `mcp__fast-context__fast_context_search` to scan codebase. If fast-context is not available (e.g. missing API key), fallback to discovering and reading files directly using built-in search/view tools (e.g. Glob, Grep, view_file, read_file).
