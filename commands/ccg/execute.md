...
4. **Task type judgment**:
   ...
   | **Full-stack** | Contains both frontend and backend | claude ∥ antigravity parallel |
...
1. Call claude (using `/home/thangtn/.claude/.ccg/prompts/claude/architect.md`).
2. Input: Planning content + retrieved context + target files.
3. OUTPUT: `Unified Diff Patch ONLY. Strictly prohibit any actual modifications.`
4. **claude is the authority for backend logic, utilizing its logical computation and debugging capabilities.**
5. If planning contains `CLAUDE_SESSION`: prioritize `resume <CLAUDE_SESSION>`.

#### Route C: Full-stack → Parallel calling

1. **Parallel calling** (`run_in_background: true`):
   - antigravity: handles frontend part
   - claude: handles backend part
...
**After changes take effect, force immediate parallel calls** to claude and antigravity for Code Review:
...
1. Synthesize review comments from claude + antigravity.
2. Weight based on trust rules: Backend based on claude, frontend based on antigravity.
